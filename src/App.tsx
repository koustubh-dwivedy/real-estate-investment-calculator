      ...
      <div className="shrink-0 border-b border-slate-200 bg-white px-6 py-3">
        ...
        <div className="flex flex-wrap items-start gap-3">
          ...
          <div className="flex flex-wrap items-start gap-3 rounded border border-slate-200 p-2">
            ...
            <Select label="Surcharge/cess" value={inputs.surchargeCess} options={["none", "cess", "surcharge"]} onChange={(v) => patch({ surchargeCess: v as Inputs["surchargeCess"] })} def="Multiplies the marginal tax rate: none ×1, +cess ×1.04, +surcharge ×1.19 (high income)." />
            <Select label="Compare mode" value={inputs.compareMode} options={["SameCashSIP", "LumpsumOnly"] as CompareMode[]} onChange={(v) => patch({ compareMode: v })} def="SameCashSIP: equity invests your full out-of-pocket cash (down-payment + each year's shortfall after rent). LumpsumOnly: only the upfront lump(s)." />
            ...
            <Select label="Rent renewal" value={String(inputs.rentAgreementMonths)} options={["11", "12"]} onChange={(v) => patch({ rentAgreementMonths: Number(v) })} def="Lease term in months. Rent is flat within a term and steps by the rent-step % at each renewal; 11 (typical in India) renews sooner than 12. Occupancy unchanged." />
